...
def loop(desc: str, root: 'Optional[TreeNode]', p: 'TreeNode', q: 'TreeNode'):
    ...
    if eq(root, p):
        ...
        if is_in_subtree(root.right, q):
            print(f"root,p,right {desc} r {val(root)} l {val(root.left)} r {val(root.right)} p {val(p)} q {val(q)}")
            return root
        if is_in_subtree(root.left, q):
            print(f"root,p,peft {desc} r {val(root)} l {val(root.left)} r {val(root.right)} p {val(p)} q {val(q)}")
            return root
